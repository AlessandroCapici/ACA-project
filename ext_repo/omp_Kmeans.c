...
double findEuclideanDistance(float *pointA, float *pointB) {
	...
	return sqrt(
			pow(((double) (*(pointB + 0)) - (double) (*(pointA + 0))), 2)
					+ pow(((double) (*(pointB + 1)) - (double) (*(pointA + 1))),
							2)
					+ pow(((double) (*(pointB + 2)) - (double) (*(pointA + 2))),
							2));
}

void threadedClustering(int tID, int N, int K, int num_threads,
		float *data_points, float **cluster_points, float **centroids,
		int *num_iterations) {
	...
	while ((delta_global > THRESHOLD) && (iteration_count < MAX_ITERATIONS)) {
		...
		for (int i = start; i < end; i++) {
			...
			for (int j = 0; j < K; j++) {
				 ...
				 */
				current_distance = findEuclideanDistance(
						(data_points + (i * 3)),
						(centroids_global + (iteration_count * K + j) * 3));
			...
			current_centroid[current_cluster_id[i - start] * 3] +=
					data_points[(i * 3)];
			current_centroid[current_cluster_id[i - start] * 3 + 1] +=
					data_points[(i * 3) + 1];
...
#pragma omp critical
		...
		for (int i = 0; i < K; i++) {
			current_delta += findEuclideanDistance(
					(centroids_global + (iteration_count * K + i) * 3),
					(centroids_global + ((iteration_count - 1) * K + i) * 3));
		}
...
#pragma omp master
	...
	for (int i = start; i < end; i++) {
		...
		cluster_points_global[i * 4 + 3] =
				(float) current_cluster_id[i - start];
	}
...
void kmeansClusteringOmp(int N, int K, int num_threads, float *data_points,
		float **cluster_points, float **centroids, int *num_iterations) {
	// 7 arguments
	// N              - Number of data points in the dataset (read only)
	// K              - Number of clusters to be created (read only)
	// num_threads    - No. of threads using which the algorithm should be executed (read only)
...
#pragma omp parallel
	{
		...
		threadedClustering(tID, N, K, num_threads, data_points, cluster_points,
	...
	*centroids = (float*) calloc(centroids_size, sizeof(float));
	...
	for (int i = 0; i < K; i++) {
		printf("Final Centroids:\t(%f, %f, %f)\n",
				*(*centroids + ((*num_iterations) * K) + (i * 3)),
				*(*centroids + ((*num_iterations) * K) + (i * 3) + 1),
				*(*centroids + ((*num_iterations) * K) + (i * 3) + 2));
	}
}
